...
def voted_perceptron(X_train, y_train, T):
    ...
    # Initialize weight vector, count vector, and unique weight set
    weights = np.zeros(X_train.shape[1])
    counts = np.zeros(1)
    unique_weights = set()
    ...
    for _ in range(T):
        ...
        for i in range(X_train.shape[0]):
            ...
            if y_i * np.dot(weights, x_i) <= 0:
                weights += y_i * x_i
                counts[0] += 1
            # Otherwise, increase counts by one and check for uniqueness
            else:
                counts = np.append(counts, 1)
                # Check if current weight vector is unique
                weight_vector_tuple = tuple(weights)
                if weight_vector_tuple not in unique_weights:
                    unique_weights.add(weight_vector_tuple)

    # Return list of (weight_vector, count) pairs
    weight_count_pairs = []
    for i in range(len(counts)):
        if counts[i] > 0:
            weight_vector_copy = weights.copy()
            weight_count_pairs.append((weight_vector_copy, counts[i]))
